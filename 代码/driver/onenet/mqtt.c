...
uint8_t MQTT_UnPacketRecv(uint8_t *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		if(strstr((int8_t *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	//如果是命令下发
			status = MQTT_PKT_CMD;
		else
...
uint1_t MQTT_PacketSaveBinData(const int8_t *name, int16_t file_len, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	bin_head = (int8_t *)MQTT_MallocBuffer(13 + strlen(name));
	if(bin_head == NULL)
		return result;
	...
	payload = (int8_t *)MQTT_MallocBuffer(payload_size - file_len);
	if(payload == NULL)
	{
...
uint8_t MQTT_UnPacketCmd(uint8_t *rev_data, int8_t **cmdid, int8_t **req, uint16_t *req_len)
{
	...
	int8_t *dataPtr = strchr((int8_t *)rev_data + 6, '/');	//加6是跳过头信息
	...
	*cmdid = (int8_t *)MQTT_MallocBuffer(37);				//cmdid固定36字节，多分配一个结束符的位置
	if(*cmdid == NULL)
		return 2;
	...
	*req = (int8_t *)MQTT_MallocBuffer(*req_len + 1);		//分配命令长度+1
	if(*req == NULL)
	{
